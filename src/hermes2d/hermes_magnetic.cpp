...
Scalar magnetic_matrix_form_real_real(int n, double *wt, Func<Real> *u_ext[], Func<Real> *u, Func<Real> *v, Geom<Real> *e, ExtData<Scalar> *ext)
{
    ...
    if (isPlanar)
        for (int i = 0; i < n; i++)
        {
            result += 1.0 / (magneticLabel[e->elem_marker].permeability.value((isLinear) ? 0.0 : sqrt(sqr(ext->fn[0]->dx[i]) + sqr(ext->fn[0]->dy[i]))) * MU0)
        ...
        {
            result += 1.0 / (magneticLabel[e->elem_marker].permeability.value((isLinear) ? 0.0 : sqrt(sqr(ext->fn[0]->dx[i]) + sqr(ext->fn[0]->dy[i]))) * MU0)
                    * (wt[i] * (v->dx[i] * u->val[i]) / e->x[i] + wt[i] * (u->dx[i] * v->dx[i] + u->dy[i] * v->dy[i]))
                    - magneticLabel[e->elem_marker].conductivity * (wt[i] * u->val[i] *
                                                                    ((magneticLabel[e->elem_marker].velocity_x - e->y[i] * magneticLabel[e->elem_marker].velocity_angular) * v->dx[i] +
                                                                     (magneticLabel[e->elem_marker].velocity_y + e->x[i] * magneticLabel[e->elem_marker].velocity_angular) * v->dy[i]));
...
template<typename Real, typename Scalar>
Scalar magnetic_jacobian(int n, double *wt, Func<Real> *u_ext[], Func<Real> *u, Func<Real> *v, Geom<Real> *e, ExtData<Scalar> *ext)
{
    Scalar result = 0;
...
template<typename Real, typename Scalar>
Scalar magnetic_residual(int n, double *wt, Func<Real> *u_ext[], Func<Real> *v, Geom<Real> *e, ExtData<Scalar> *ext)
{
    Scalar result = 0;
    Func<Scalar>* u_prev = u_ext[0];
...
void callbackMagneticWeakForm(WeakForm *wf, Hermes::vector<Solution *> slnArray)
{
    if (slnArray.size() == 1)
    {
        ...
        if (Util::scene()->problemInfo()->linearityType == LinearityType_Linear)
        {
            if (analysisType == AnalysisType_Transient)
            {
                ...
                wf->add_vector_form(0, callback(magnetic_vector_form_real), HERMES_ANY, Hermes::vector<MeshFunction *>(slnArray.at(0), slnArray.at(0)));
        ...
        else if (Util::scene()->problemInfo()->linearityType == LinearityType_Picard)
        {
            if (analysisType == AnalysisType_Transient)
            {
                wf->add_matrix_form(0, 0, callback(magnetic_matrix_form_real_real));
                wf->add_vector_form(0, callback(magnetic_vector_form_real), HERMES_ANY, Hermes::vector<MeshFunction *>(slnArray.at(0), slnArray.at(1)));
                wf->add_vector_form_surf(0, callback(magnetic_vector_form_surf_real));
            }
            else
            {
        ...
        else if (Util::scene()->problemInfo()->linearityType == LinearityType_Newton)
        {
            if (analysisType == AnalysisType_Transient)
            {
            }
            ...
            {
                wf->add_matrix_form(0, 0, callback(magnetic_jacobian));
                wf->add_vector_form(0, callback(magnetic_residual));
                wf->add_vector_form_surf(0, callback(magnetic_vector_form_surf_real));
            }
        }
    }
    else
    {
        ...
        {
            wf->add_matrix_form(0, 0, callback(magnetic_matrix_form_real_real), HERMES_NONSYM, HERMES_ANY, Hermes::vector<MeshFunction *>(slnArray.at(0), slnArray.at(1)));
            wf->add_matrix_form(1, 1, callback(magnetic_matrix_form_imag_imag), HERMES_NONSYM, HERMES_ANY, Hermes::vector<MeshFunction *>(slnArray.at(0), slnArray.at(1)));
        }
...
void HermesMagnetic::writeLabelMarkerToDomElement(QDomElement *element, SceneLabelMarker *marker)
{
    ...
    element->setAttribute("permeability", labelMagneticMarker->permeability.toString());
...
QList<SolutionArray *> HermesMagnetic::solve(ProgressItemSolve *progressItemSolve)
{
    ...
    for (int i = 0; i<Util::scene()->labels.count(); i++)
    {
        ...
        else
        {
            ...
            magneticLabel[i].permeability = labelMagneticMarker->permeability;
    ...
    if (Util::scene()->sceneSolution()->isSolved())
    {
        ...
        if (marker != NULL)
        {
                    ...
                    Util::scene()->problemInfo()->analysisType == AnalysisType_Transient)
            {
                ...
                permeability = marker->permeability.value(sqrt(sqr(B_real.x) + sqr(B_real.y)));
                ...
                H_real = B_real / (permeability * MU0);
                ...
                // energy density
                wm = 0.5 * (sqr(B_real.x) + sqr(B_real.y) + sqr(B_imag.x) + sqr(B_imag.y)) / (permeability * MU0);
            }
            ...
            if (Util::scene()->problemInfo()->analysisType == AnalysisType_Harmonic)
            {
                ...
                permeability = marker->permeability.value(sqrt(sqr(B_real.x) + sqr(B_real.y) + sqr(B_imag.x) + sqr(B_imag.y)));
                conductivity = marker->conductivity.number();
                remanence = marker->remanence.number();
                remanence_angle = marker->remanence_angle.number();
                velocity = Point(marker->velocity_x.number() - marker->velocity_angular.number() * point.y,
                ...
                // electric displacement
                H_real = B_real / (permeability * MU0);
                H_imag = B_imag / (permeability * MU0);
                ...
                // energy density
                wm = 0.25 * (sqr(B_real.x) + sqr(B_real.y) + sqr(B_imag.x) + sqr(B_imag.y)) / (permeability * MU0);
            }
        }
    }
}
...
void SurfaceIntegralValueMagnetic::calculateVariables(int i)
{
    ...
    if (fabs(marker->permeability.number() - 1.0) < EPS_ZERO)
    {
        ...
        if (Util::scene()->problemInfo()->problemType == ProblemType_Planar)
        {
            ...
            forceMaxwellX -= pt[i][2] * tan[i][2] * 1.0 / (MU0 * marker->permeability.value(sqrt(sqr(Bx) + sqr(By)))) *
                    (Bx * (nx * Bx + ny * By) - 0.5 * nx * (sqr(Bx) + sqr(By)));

            forceMaxwellY -= pt[i][2] * tan[i][2] * 1.0 / (MU0 * marker->permeability.value(sqrt(sqr(Bx) + sqr(By)))) *
                    (By * (nx * Bx + ny * By)
        ...
        else
        {
            ...
            forceMaxwellY -= 2 * M_PI * x[i] * pt[i][2] * tan[i][2] * 1.0 / (MU0 * marker->permeability.value(sqrt(sqr(Bx) + sqr(By)))) *
...
void VolumeIntegralValueMagnetic::calculateVariables(int i)
{
    ...
    if (Util::scene()->problemInfo()->problemType == ProblemType_Planar)
    {
        if (Util::scene()->problemInfo()->analysisType == AnalysisType_Harmonic)
        {
            h1_integrate_expression(0.25 * (sqr(dudx1[i]) + sqr(dudy1[i]) + sqr(dudx2[i]) + sqr(dudy2[i]))
                                    / (marker->permeability.value(sqrt(sqr(dudx1[i]) + sqr(dudy1[i]) + sqr(dudx2[i]) + sqr(dudy2[i]))) * MU0));
        }
        else
        {
            h1_integrate_expression(0.5 * (sqr(dudx1[i]) + sqr(dudy1[i]))
                                    / (marker->permeability.value(sqrt(sqr(dudx1[i]) + sqr(dudy1[i]))) * MU0));
        }
    }
    else
    {
        if (Util::scene()->problemInfo()->analysisType == AnalysisType_Harmonic)
        {
            h1_integrate_expression((2 * M_PI * x[i] * 0.25 * sqr(sqrt(sqr(dudy1[i]) + sqr(dudx1[i] + ((x[i] > 0) ? value1[i] / x[i] : 0.0))))
                                     / (marker->permeability.value(sqrt(sqr(dudx1[i]) + sqr(dudy1[i]) + sqr(dudx2[i]) + sqr(dudy2[i]))) * MU0))
                                    + (2 * M_PI * x[i] * 0.25 * sqr(sqrt(sqr(dudy2[i]) + sqr(dudx2[i] + ((x[i] > 0) ? value2[i] / x[i] : 0.0))))
                                       / (marker->permeability.value(sqrt(sqr(dudx1[i]) + sqr(dudy1[i]) + sqr(dudx2[i]) + sqr(dudy2[i]))) * MU0)));
        }
        else
        {
            h1_integrate_expression(2 * M_PI * x[i] * 0.5 * sqr(sqrt(sqr(dudy1[i]) + sqr(dudx1[i] + ((x[i] > 0) ? value1[i] / x[i] : 0.0))))
                                    / (marker->permeability.value(sqrt(sqr(dudx1[i]) + sqr(dudy1[i]))) * MU0));
        }
    }
    energy += result;
...
{
    txtPermeability = new SLineEditValue(this, (Util::scene()->problemInfo()->linearityType == LinearityType_Linear));
